...
ARGF.lines do |line|
  ...
  if matches
    ...
    record = [
      druid,
      load_real,
      format('%0.3f', (100 * (load_real.to_f - load_cpu.to_f) / load_real.to_f)),
      solr_real,
      format('%0.3f', (100 * (solr_real.to_f - solr_cpu.to_f) / solr_real.to_f)),
      format('%0.6f', (load_real.to_f + solr_real.to_f))
    ]
    puts record.join(',')
  end
end
